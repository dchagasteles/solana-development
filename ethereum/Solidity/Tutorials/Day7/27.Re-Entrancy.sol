...
Reentracy exploits allows B to call back into A before A finishes execution.
...
/**
  typical reenetracy attack work flow
 */
interface I_C{
    function D() external;
}
...
contract A {
    address c; //contract c's address

    function E() public {
        //E function is called two times. Reentrancy attack!!
    }

    //user calls B function
    function B() public {
        E(); //first call
        I_C(c).D();
    }
